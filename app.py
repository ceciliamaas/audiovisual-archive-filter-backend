...
from search import search  # Import from app directory
...
# Resolve the path relative to the script's location (app directory)
input_videos_dir = Path(__file__).parent / "app" / "input_videos"
...
FRAME_PATHS_PATH = Path(__file__).parent / "app/embeddings/frame_paths.pkl"
OBJECT_PATHS_PATH = Path(__file__).parent / "app/embeddings/object_paths.pkl"
        ...
        for result in results:
            ...
            with col2:
                if result["type"] == "object":
                    frame_path = Path("app/output_frames") / result["frame_path"]
