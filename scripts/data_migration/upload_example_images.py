...
def upload_example_images(force=False):
    """Upload example images to S3"""
    ...
    print(f"\nStorage backend: {storage_info.get('backend_type')}")
    ...
    for image_file in image_files:
        ...
        try:
            ...
            if not force and storage.file_exists(s3_path):
                print(
                    f"  ⚠️  Already exists in S3, skipping... (use --force to overwrite)"
                )
                skipped_count += 1
                continue

            if force and storage.file_exists(s3_path):
                print(f"  🔄 File exists, but --force specified, uploading anyway...")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--list", action="store_true", help="List example images in S3")
    parser.add_argument(
        "--force", action="store_true", help="Force upload even if files exist in S3"
    )
    ...
    if args.list:
        ...
        success = upload_example_images(force=args.force)
