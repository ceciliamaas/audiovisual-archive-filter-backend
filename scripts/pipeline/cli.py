    ...
    for state in states:
        status_emoji = (
            ...
            if state.status == VideoStatus.COMPLETED
                ...
                else "✗" if state.status == VideoStatus.FAILED else "⏸"
            )
        )
        print(
...
def main():
    ...
    process_parser.add_argument(
        "--source",
        required=True,
        choices=["youtube", "drive", "local"],
        help="Source type",
    )
    process_parser.add_argument("--url", required=True, help="URL or path to video")
    process_parser.add_argument(
        "--steps", help="Comma-separated list of steps to run (default: all)"
    )
    process_parser.add_argument(
        "--force", action="store_true", help="Reprocess even if already completed"
    )
    process_parser.add_argument(
        "--fps", type=int, default=1, help="Frames per second to extract (default: 1)"
    )
    process_parser.add_argument(
        "--yolo-classes", help="Comma-separated YOLO classes to detect"
    )
    process_parser.add_argument(
        "--confidence",
        type=float,
    ...
    process_parser.add_argument(
        "--iou", type=float, default=0.3, help="YOLO IOU threshold (default: 0.3)"
    )
    ...
    validate_parser = subparsers.add_parser(
        "validate", help="Validate artifacts for a video"
    )
