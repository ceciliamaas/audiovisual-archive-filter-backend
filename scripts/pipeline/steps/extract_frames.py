...
class ExtractFramesStep(PipelineStep):
    ...
    def execute(self) -> bool:
        ...
        print(f"    Video FPS: {video_fps:.2f}, extracting every {frame_interval} frames")
    ...
    def validate_output(self) -> tuple[bool, Optional[str]]:
        ...
        if self.state.frame_count != len(frame_files):
            return False, f"Frame count mismatch: expected {self.state.frame_count}, found {len(frame_files)}"
