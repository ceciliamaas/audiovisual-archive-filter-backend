    ...
    def execute(self) -> bool:
        ...
        if objects_dir.exists():
            ...
            for i, obj_path in enumerate(object_files):
                ...
                obj_key = NamingConvention.object_s3_key(self.video_name, frame_idx, obj_idx)
                if not storage.upload_file(str(obj_path), obj_key):
                    print(f"    Warning: Failed to upload {obj_path.name}")
                
                if (i + 1) % 10 == 0:
                    print(f"    Uploaded {i+1}/{len(object_files)} objects...", end="\r")
        ...
        storage.upload_file(str(frame_emb_path), NamingConvention.frame_embeddings_s3_key())
        storage.upload_file(str(frame_paths_path), NamingConvention.frame_paths_s3_key())
        ...
        if obj_emb_path.exists() and obj_paths_path.exists():
            storage.upload_file(str(obj_emb_path), NamingConvention.object_embeddings_s3_key())
            storage.upload_file(str(obj_paths_path), NamingConvention.object_paths_s3_key())
