...
async def search_by_text(request: TextSearchRequest):
    ...
    """
    try:
        logger.info(f"Text search request: '{request.query}' (max_results={request.max_results})")
...
async def search_by_image(
    ...
    try:
        ...
        logger.info(f"Image search request: {image.filename} (max_results={max_results})")
...
async def get_example_images():
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Failed to get example images: {str(e)}"
        )
